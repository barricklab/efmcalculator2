...
def seq_parser(filename, output, df):
    ...
    for record in sequences:
        ...
        seq_len=len(seq)
        build_sub_seq_from_seq(seq, df, seq_len)
    df.sort_values(['Size'], ascending=[True]).to_csv(output, index=False)


def build_sub_seq_from_seq(seq, df, seq_len):
    ...
    for i, letter in enumerate(seq):
        for j in range(MAX_SHORT_SEQ_LEN, MIN_SHORT_SEQ_LEN, -1):
            if len(seq[i:i + j]) > MIN_SHORT_SEQ_LEN:
                sub_seq = seq[i:i + j]
                rem_seq = seq[i + 1:]
                find_short_seq(seq, sub_seq, df, seq_len)
...
def find_short_seq(seq, sub_seq, df, seq_len):
