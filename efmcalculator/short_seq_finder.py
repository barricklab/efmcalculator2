...
def find_short_seq(seq, sub_seq, df, seq_len, isCircular):
    ...
    if count > 1 and str(sub_seq) not in visited_sequences:
        ...
        for seq_attr in sss.scan_short_sequence(seq, sub_seq, seq_len,isCircular, count):
                logger.debug("Sequence = {} : Size = {} : Distance = {} : Start-Pos = {} : End-Pos = {}".format(
                    seq_attr.sub_seq, seq_attr.length, seq_attr.distance, seq_attr.start_pos, seq_attr.end_pos))
                df.loc[seq_attr.sub_seq + '~' + str(seq_attr.start_pos)] = [seq_attr.sub_seq, seq_attr.length,
                                                                            seq_attr.distance,
...
if __name__ == '__main__':
    ...
    main(filename=args.input_file_name, output=args.output_csv_file_name, isCircular=args.isCircular)
